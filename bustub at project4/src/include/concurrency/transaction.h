 ...
 *
 * Read uncommitted:
 * Read uncommited allows uncommitted data to be read.
 ...
 * Obtain all locks first;plus S locks are released immediately, X locks are only released after a commit
 ...
 * Phantoms may happen.
 * Obtain all locks first;plus strict 2PL
 * The transaction is required to take all locks.
 * All locks are allowed in the GROWING state
...
enum class AbortReason {
  /** It attempted to acquire a lock on a resource (like a table or row) that is in the process of being shrunk*/
  LOCK_ON_SHRINKING,
  /** a conflict arising from an attempt to upgrade a lock,
   * perhaps from a shared to an exclusive lock, while another transaction holds a conflicting lock type.*/
  UPGRADE_CONFLICT,
  /** a transaction tries to place a shared lock on a resource in a "READ UNCOMMITTED" isolation level,
   * where generally no locks should be held because this level allows reading uncommitted changes.*/
  LOCK_SHARED_ON_READ_UNCOMMITTED,
  /** if a transaction tries to operate on a table assuming a certain lock is in place,
   * but finds that the lock is not actually present.*/
  TABLE_LOCK_NOT_PRESENT,
  /** it tried to apply an intention lock directly on a row, which is typically not allowed.*/
  ATTEMPTED_INTENTION_LOCK_ON_ROW,
  /** the table was unlocked before individual row-level locks were released,
   * which violates the proper locking protocol that ensures data integrity.*/
  TABLE_UNLOCKED_BEFORE_UNLOCKING_ROWS,
  /** if a lock upgrade request was made (e.g., from shared to exclusive)
   * but was found to be incompatible with existing locks or the current transaction state.*/
  INCOMPATIBLE_UPGRADE,
  /** a transaction attempted to release a lock that it never acquired,
   * which could point to a logic error in the transaction's lock management.*/
