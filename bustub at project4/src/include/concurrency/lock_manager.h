...
class LockManager {
 public:
  ...
  /**
   * Removes all edges from t1 to all other transactions in the graph.
   * @param t1 transaction waiting for a lock
   */
  auto DeleteNode(txn_id_t t1) -> void;

  void StartCycleDetection();

  void StopCycleDetection();
  ...
  auto DFS(txn_id_t txn_id, std::unordered_set<txn_id_t> &visited, std::unordered_set<txn_id_t> &rec_stack) -> bool;
  ...
  /**
   * Inserts or deletes a lock request from the table lock set of a transaction.
   * @param txn the transaction
   * @param lock_request the lock request
   * @param insert true if inserting, false if deleting
   */
  auto InsertOrDeleteTableLockSet(Transaction *txn, std::shared_ptr<LockRequest> &lock_request, EditType edit) -> void;

  auto InsertOrDeleteRowLockSet(Transaction *txn, const std::shared_ptr<LockRequest> &lock_request, EditType edit)
      -> void;

  /**
   * Check the Lock to grant*/
 ...
 private:
  ...
  /** Transaction in the graph that are safe */
  std::set<txn_id_t> safe_set_;
  /** Transaction in the graph that are active */
