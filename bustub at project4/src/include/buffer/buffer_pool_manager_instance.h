...
class BufferPoolManagerInstance : public BufferPoolManager {
 ...
 protected:
  ...
  auto NewPgImp(page_id_t *page_id) -> Page * override {
    ...
    frame_id_t newframe;
    if (free_list_.empty() && !(replacer_->Evict(&newframe))) {
      return nullptr;
    }
    if (!(free_list_.empty())) {  // if free list is not empty
      newframe = free_list_.front();
      free_list_.pop_front();
    } else {  // if free list is empty, evict a frame
      Page *evicted_page = &pages_[newframe];
      ...
      page_address_map_.erase(evicted_page_id);
      evicted_page->is_dirty_ = false;
      evicted_page->ResetMemory();
      page_table_->Remove(evicted_page_id);
    }
    *page_id = AllocatePage();
    page_table_->Insert(*page_id, newframe);
    pages_[newframe].page_id_ = *page_id;
    pages_[newframe].pin_count_ = 1;
    replacer_->RecordAccess(newframe);
    replacer_->SetEvictable(newframe, false);
    page_address_map_[*page_id] = &pages_[newframe];
  ...
  auto FetchPgImp(page_id_t page_id) -> Page * override {
    ...
    if (!(page_table_->Find(page_id, frame_id))) {
      ...
      page_address_map_[page_id] = &pages_[frame_id];
  ...
  void FlushAllPgsImp() override {
    std::scoped_lock<std::mutex> lock(latch_);
    std::list<frame_id_t> cash_frame_list = replacer_->GetCashList();
    std::list<page_id_t> cash_page_list;
    for (auto &it_frame : cash_frame_list) {
      cash_page_list.push_back(pages_[it_frame].page_id_);
    }
    for (auto &it_page : cash_page_list) {
      FlushPgImp(it_page);
    }
  };
   ...
   * and add the frame back to the free list.
   * Also, reset the page's memory and metadata. Finally, you should call DeallocatePage() to
  ...
  auto DeletePgImp(page_id_t page_id) -> bool override {
    ...
    page_address_map_.erase(page_id);
  ...
  Page *pages_;
  /** Map from page_id_t to the address of the pages_[frame_id] */
  std::unordered_map<page_id_t, Page *> page_address_map_;
