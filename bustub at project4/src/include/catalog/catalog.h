...
class Catalog {
 public:
  ...
  auto CreateTable(Transaction *txn, const std::string &table_name, const Schema &schema, bool create_table_heap = true)
    ...
    if (table_names_.count(table_name) != 0) {
      return NULL_TABLE_INFO;  // Table names should be unique!
    }
    ...
    /**
     * The emplace method is used to insert a new element into the map.
     * It constructs the element in-place, avoiding unnecessary copy operations.
     * std::move(meta) transfers ownership of the meta object into the map,
     * which means after this operation,
     * the meta object is incorporated into the map and no longer exists independently in the function.
     * */
  ...
  auto CreateIndex(Transaction *txn, const std::string &index_name, const std::string &table_name, const Schema &schema,
    ...
    for (auto tuple = heap->Begin(txn); tuple != heap->End(); ++tuple) {
      /**
       * The function KeyFromTuple is a utility method for extracting a subset of attributes from a tuple
       * to create a new "key" tuple based on a specified schema (key_schema).
       * */
 ...
 private:
  ...
  /**
   * std::atomic is a template in C++ that provides a mechanism to manage types that
   * can be safely read from and written to from different threads without causing data races.
   * Here are the key features of std::atomic:
   *
   * Thread Safety:
   * Operations on an std::atomic variable are atomic, meaning they are performed as a single,
   * uninterruptible operation. This is crucial in multithreaded applications where multiple threads might need to
   * read or modify the same variable concurrently.
   *
   * Memory Ordering:
   * std::atomic provides guarantees about memory ordering for operations on atomic variables.
   * This helps in properly synchronizing the program even on systems with complex memory models.
   *
   * Lock-Free:
   * In many cases, operations on atomic variables are implemented in a lock-free manner,
   * meaning they do not use mutexes or other locking mechanisms.
   * This can lead to performance improvements in multithreaded applications.
   * */
