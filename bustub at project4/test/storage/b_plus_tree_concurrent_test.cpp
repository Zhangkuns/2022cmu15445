...
// helper function to insert
void InsertHelper2(BPlusTree<GenericKey<8>, RID, GenericComparator<8>> *tree, const std::vector<int64_t> &keys,
                   uint64_t tid, __attribute__((unused)) uint64_t thread_itr = 0) {
  GenericKey<8> index_key;
  RID rid;
  // create transaction
...
void InsertAndGetHelper2(BPlusTree<GenericKey<8>, RID, GenericComparator<8>> *tree, const std::vector<int64_t> &keys,
                         uint64_t tid, __attribute__((unused)) uint64_t thread_itr = 0) {
  GenericKey<8> index_key;
  RID rid;
  // create transaction
...
void IterateHelper2(BPlusTree<GenericKey<16>, RID, GenericComparator<16>> &tree) {
...
void DeleteAndGetHelper2(BPlusTree<GenericKey<8>, RID, GenericComparator<8>> *tree,
                         const std::vector<int64_t> &remove_keys, uint64_t tid,
                         __attribute__((unused)) uint64_t thread_itr = 0) {
  GenericKey<8> index_key;
  // create transaction
  Transaction *transaction = new Transaction(tid);
 ...
 */
TEST(BPlusTreeTestC2Con, DeleteTest3) {
  TEST_TIMEOUT_BEGIN
  DeleteTest1Call();
  remove("test.db");
  remove("test.log");
 ...
 * without overlap.
 */
TEST(BPlusTreeTestC2Con, DeleteTest4) {
  TEST_TIMEOUT_BEGIN
  DeleteTest2Call();
  remove("test.db");
  remove("test.log");
 ...
 */
TEST(BPlusTreeTestC2Con, MixTest5) {
  TEST_TIMEOUT_BEGIN
  MixTest1Call();
  remove("test.db");
  remove("test.log");
 ...
 * inserted.
 */
TEST(BPlusTreeTestC2Con, MixTest6) {
  TEST_TIMEOUT_BEGIN
  MixTest2Call();
  remove("test.db");
  remove("test.log");
 ...
 * deleted and new keys are added correctly.
 */
TEST(BPlusTreeTestC2Con, MixTest7) {
  TEST_TIMEOUT_BEGIN
  MixTest3Call();
  remove("test.db");
  remove("test.log");
...
TEST(BPlusTreeTestC2Con, MixTest8) {
  TEST_TIMEOUT_BEGIN
  MixTest4Call();
  remove("test.db");
  remove("test.log");
 ...
 */
TEST(BPlusTreeConcurrentTestC2Seq, InsertTest9) {
  // create KeyComparator and index schema
  auto key_schema = ParseCreateStatement("a bigint");
  GenericComparator<8> comparator(key_schema.get());
 ...
 * but added iterator for value checking
 */
TEST(BPlusTreeConcurrentTestC2Seq, InsertTest10) {
  // create KeyComparator and index schema
  auto key_schema = ParseCreateStatement("a bigint");
  GenericComparator<8> comparator(key_schema.get());
 ...
 */
TEST(BPlusTreeConcurrentTestC2Seq, DeleteTest11) {
  // create KeyComparator and index schema
 ...
 */
TEST(BPlusTreeConcurrentTestC2Seq, DeleteTest12) {
  // create KeyComparator and index schema
  auto key_schema = ParseCreateStatement("a bigint");
  GenericComparator<8> comparator(key_schema.get());
 ...
 */
TEST(BPlusTreeConcurrentTestC2Seq, ScaleTest13) {
  // create KeyComparator and index schema
  auto key_schema = ParseCreateStatement("a bigint");
  GenericComparator<8> comparator(key_schema.get());
 ...
 * inserted.
 */
TEST(BPlusTreeConcurrentTestC2Seq, SequentialMixTest14) {
  // create KeyComparator and index schema
  auto key_schema = ParseCreateStatement("a bigint");
  GenericComparator<8> comparator(key_schema.get());
...
TEST(BPlusTreeConcurrentTest, InsertTest15) {
  for (size_t iter = 0; iter < NUM_ITERS_DEBUG; iter++) {
    // create KeyComparator and index schema
    auto key_schema = ParseCreateStatement("a bigint");
    GenericComparator<8> comparator(key_schema.get());
...
TEST(BPlusTreeConcurrentTest, InsertAndGetTest16) {
  for (size_t iter = 0; iter < NUM_ITERS_DEBUG; iter++) {
    // create KeyComparator and index schema
    auto key_schema = ParseCreateStatement("a bigint");
    GenericComparator<8> comparator(key_schema.get());
    ...
    LaunchParallelTest(4, 0, InsertAndGetHelper2, &tree, keys);
...
TEST(BPlusTreeConcurrentTest, DeleteAndGetTest17) {
  for (size_t iter = 0; iter < NUM_ITERS_DEBUG; iter++) {
    // create KeyComparator and index schema
    auto key_schema = ParseCreateStatement("a bigint");
    GenericComparator<8> comparator(key_schema.get());
    ...
    LaunchParallelTest(2, 1, DeleteAndGetHelper2, &tree, remove_keys);
...
TEST(BPlusTreeConcurrentTest, DeleteTest18) {
  for (size_t iter = 0; iter < NUM_ITERS_DEBUG; iter++) {
    // create KeyComparator and index schema
    auto key_schema = ParseCreateStatement("a bigint");
    GenericComparator<8> comparator(key_schema.get());
...
TEST(BPlusTreeConcurrentTest, DeleteTest19) {
  for (size_t iter = 0; iter < NUM_ITERS_DEBUG; iter++) {
    // create KeyComparator and index schema
    auto key_schema = ParseCreateStatement("a bigint");
    GenericComparator<8> comparator(key_schema.get());
    ...
    LaunchParallelTest(3, 1, DeleteHelperSplit, &tree, remove_keys, 3);
...
TEST(BPlusTreeConcurrentTest, MixTest20) {
  // create KeyComparator and index schema
  auto key_schema = ParseCreateStatement("a bigint");
  GenericComparator<8> comparator(key_schema.get());
  ...
  LaunchParallelTest(1, 1, InsertHelper, &tree, keys);
  // concurrent delete
  std::vector<int64_t> remove_keys = {1, 4, 3, 5, 6};
  LaunchParallelTest(1, 2, DeleteHelper, &tree, remove_keys);
...
TEST(BPlusTreeConcurrentTest, MixTest21) {
  for (size_t iter = 0; iter < NUM_ITERS_DEBUG; iter++) {
    // create KeyComparator and index schema
    auto key_schema = ParseCreateStatement("a bigint");
    GenericComparator<8> comparator(key_schema.get());
    ...
    // concurrent insert
    LaunchParallelTest(4, 0, InsertHelperSplit, &tree, all_deleted, 4);
    // concurrent insert and delete
    std::thread t0(InsertHelper, &tree, keys, 0, 0);
    LaunchParallelTest(4, 4, DeleteHelperSplit, &tree, all_deleted, 4);
...
TEST(BPlusTreeConcurrentTest, MixTest22) {
  for (size_t iter = 0; iter < NUM_ITERS_DEBUG; iter++) {
    ...
    // concurrent insert
    LaunchParallelTest(4, 0, InsertHelperSplit, &tree, deleted1, 4);
    LaunchParallelTest(4, 4, InsertHelperSplit, &tree, deleted2, 4);
    // concurrent insert and delete
    auto tree_size = tree.GetSize();
    EXPECT_EQ(tree_size, scale / 2);

    std::thread t0(InsertAndGetHelper2, &tree, keys, 0, 0);
    std::thread t1(InsertAndGetHelper2, &tree, keys2, 1, 0);
    std::thread t2(DeleteAndGetHelper2, &tree, deleted1, 2, 0);
    std::thread t3(DeleteAndGetHelper2, &tree, deleted2, 3, 0);
