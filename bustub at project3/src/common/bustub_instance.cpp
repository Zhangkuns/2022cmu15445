 ...
 *
 * Parser :
 * The parser stage takes the SQL query as input and checks it for syntactical correctness.
 * It breaks down the SQL statement into recognizable elements (tokens), ensuring the query follows the rules of SQL
 * grammar. The output of the parser is often a parse tree, which represents the syntactical structure of the SQL
 * statement.
 *
 * Binder :
 * The binder associates, or "binds", the elements of the query to the actual objects within the database schema.
 * For example, it will match table and column names in the query to the actual tables and columns in the database.
 * This stage ensures that the objects referenced in the query exist and that the query has the necessary permissions
 * to access them.
 *
 * Planner :
 * The planner takes the information from the binder and decides on a strategy to execute the query.
 * It considers various possible ways to perform the operations required by the query and
 * chooses the most efficient execution plan based on query cost estimation.
 * This plan details the specific operations (like joins, filters, and aggregations), their order,
 * nd how to access the data (indexes, full table scans, etc.).
 *
 * Optimizer :
 * The optimizer is a part of or closely related to the planner.
 * It may iterate over different possible plans, using statistics about the data,
 * to improve the execution strategy for the query.
 * In some systems, the planner and optimizer may be combined into one component that performs both planning and
 * optimization.
 * */
