...
TEST(LRUKReplacerTest, BasicSingleAccess2) {
  LRUKReplacer lru_replacer(5, 2);  // Buffer size of 5, K value of 2

  // Add five elements with single access history
  for (int frame_id = 1; frame_id <= 5; ++frame_id) {
    lru_replacer.RecordAccess(frame_id);
    lru_replacer.SetEvictable(frame_id, true);
  }
  ASSERT_EQ(5, lru_replacer.Size());

  // Evict frames and check order (should be in FIFO order due to single access)
  int value;
  for (int expected_frame_id = 1; expected_frame_id <= 5; ++expected_frame_id) {
    ASSERT_TRUE(lru_replacer.Evict(&value));
    ASSERT_EQ(expected_frame_id, value);
  }
  ASSERT_EQ(0, lru_replacer.Size());
}

TEST(LRUKReplacerTest, HandleNonEvictableFrames3) {
  LRUKReplacer lru_replacer(5, 2);

  // Add five elements, make frame 3 non-evictable
  for (int frame_id = 1; frame_id <= 5; ++frame_id) {
    lru_replacer.RecordAccess(frame_id);
    bool evictable = (frame_id != 3);
    lru_replacer.SetEvictable(frame_id, evictable);
  }
  ASSERT_EQ(4, lru_replacer.Size());  // Frame 3 should not be counted

  // Evict frames and check that frame 3 is not evicted
  int value;
  for (int i = 0; i < 4; ++i) {
    ASSERT_TRUE(lru_replacer.Evict(&value));
    ASSERT_NE(3, value);  // Frame 3 should not be evicted
  }
  ASSERT_EQ(0, lru_replacer.Size());
}

TEST(LRUKReplacerTest, MultipleAccessHistories4) {
  LRUKReplacer lru_replacer(4, 2);

  // Add four elements with multiple accesses for some
  lru_replacer.RecordAccess(1);  // Accessed once
  lru_replacer.RecordAccess(2);  // Accessed twice
  lru_replacer.RecordAccess(2);
  lru_replacer.RecordAccess(3);  // Accessed three times
  lru_replacer.RecordAccess(3);
  lru_replacer.RecordAccess(3);
  lru_replacer.RecordAccess(4);  // Accessed once
  for (int frame_id = 1; frame_id <= 4; ++frame_id) {
    lru_replacer.SetEvictable(frame_id, true);
  }
  ASSERT_EQ(4, lru_replacer.Size());

  // Evict frames and check order based on access frequency
  int value;
  ASSERT_TRUE(lru_replacer.Evict(&value));  // Expect frame 1 (least frequently accessed)
  ASSERT_EQ(1, value);
  ASSERT_TRUE(lru_replacer.Evict(&value));  // Expect frame 4 (next least frequently accessed)
  ASSERT_EQ(4, value);
  ASSERT_TRUE(lru_replacer.Evict(&value));  // Expect frame 2
  ASSERT_EQ(2, value);
  ASSERT_TRUE(lru_replacer.Evict(&value));  // Expect frame 3 (most frequently accessed)
  ASSERT_EQ(3, value);
  ASSERT_EQ(0, lru_replacer.Size());
}

TEST(LRUKReplacerTest, EvictTest5) {
...
TEST(LRUKReplacerTest, EvictTest6) {
  LRUKReplacer lru_replacer(3, 3);
  frame_id_t frame_id;
...
TEST(LRUKReplacerTest, SampleTest7) {
  LRUKReplacer lru_replacer(7, 2);
...
TEST(LRUKReplacerTest, ConcurrencyTest10) {  // NOLINT
  // 1/4 page has one access history, 1/4 has two accesses, 1/4 has three, and 1/4 has four
  LRUKReplacer lru_replacer(1000, 3);
