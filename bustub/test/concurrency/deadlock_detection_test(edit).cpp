...
namespace bustub {
TEST(LockManagerDeadlockDetectionTest, EdgeTest1) {
  ...
  for (int i = 0; i < num_nodes; i += 2) {
    auto edge_list_size = lock_mgr.GetEdgeList().size();
    EXPECT_EQ(i / 2, edge_list_size);
    auto t1 = txn_ids[i];
    auto t2 = txn_ids[i + 1];
    lock_mgr.AddEdge(t1, t2);
    edges.emplace_back(t1, t2);
    edge_list_size = lock_mgr.GetEdgeList().size();
    EXPECT_EQ((i / 2) + 1, edge_list_size);
  }
...
TEST(LockManagerDeadlockDetectionTest, BasicDeadlockDetectionTest3) {
  LockManager lock_mgr{};
  TransactionManager txn_mgr{&lock_mgr};
...
TEST(LockManagerDeadlockDetectionTest, CycleTest4) {
  LockManager lock_mgr{};
